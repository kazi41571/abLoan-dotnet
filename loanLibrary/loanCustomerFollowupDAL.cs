    ...
    public class loanCustomerFollowupDAL
    {
        ...
        private bool SetClassPropertiesFromSqlDataReader(SqlDataReader sqlRdr)
        {
            if (sqlRdr.Read())
            {
                ...
                if (sqlRdr["linktoGuarantorMasterId"] != DBNull.Value)
                { 
                    this.linktoGuarantorMasterId = Convert.ToInt32(sqlRdr["linktoGuarantorMasterId"]);
                    this.GuarantorName = Convert.ToString(sqlRdr["GuarantorName"]);
                    this.GuarantorIdNo = Convert.ToString(sqlRdr["GuarantorIdNo"]);
                }
        ...
        private List<loanCustomerFollowupDAL> SetListPropertiesFromSqlDataReader(SqlDataReader sqlRdr)
        {
            ...
            while (sqlRdr.Read())
            {
                ...
                if (sqlRdr["linktoGuarantorMasterId"] != DBNull.Value)
                {
                    objCustomerFollowup.linktoGuarantorMasterId = Convert.ToInt32(sqlRdr["linktoGuarantorMasterId"]);
                    objCustomerFollowup.GuarantorName = Convert.ToString(sqlRdr["GuarantorName"]);
                    objCustomerFollowup.GuarantorIdNo = Convert.ToString(sqlRdr["GuarantorIdNo"]);
