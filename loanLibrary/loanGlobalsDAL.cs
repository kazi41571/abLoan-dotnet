...
namespace loanLibrary
{
    ...
    public class loanGlobalsDAL
    {
        ...
        #region Email
        public static async Task<bool> SendEmailAsync(string ToEmail, string sub, string msg)
        {
            return await SendEmailBySendGridAsync(ToEmail, sub, msg, null);
        }
        ...
        public static async Task<bool> SendEmailBySendGridAsync(string toEmail, string subject, string message, string attachmentFileName)
        {
            // Ensure the appropriate SSL/TLS protocol
            ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls12;

            var _httpClient = new HttpClient();
            var key = System.Configuration.ConfigurationManager.AppSettings["SENDGRID_KEY"];
            _httpClient.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", key);

            // Increase timeout as necessary
            ...
            var emailData = new
            {
                ...
                from = new { email = "noreply@e2community.org", name = "Abloan" },
                subject = subject,
                content = new[] {
            ...
            string jsonPayload = JsonConvert.SerializeObject(emailData);
            StringContent content = new StringContent(jsonPayload, Encoding.UTF8, "application/json");

            try
            {
                HttpResponseMessage response = await _httpClient.PostAsync("https://api.sendgrid.com/v3/mail/send", content);
            ...
            {
                Console.WriteLine($"An HttpRequestException occurred: {httpRequestException.Message}");
                return false;
            }
            catch (Exception ex)
            {
