...
namespace abLOAN
{
    public partial class item : BasePage
    {
        ...
        protected void btnDownload_Click(object sender, EventArgs e)
        {
            ...
            objItemMasterDAL.IsEnabled = ddlFilterIsEnabled.SelectedValue == "Yes";
            objItemMasterDAL.linktoCompanyMasterId = ((loanUser)Session[loanSessionsDAL.UserSession]).CompanyMasterId;

            int TotalRecords;
            List<loanItemMasterDAL> lstItemMaster = objItemMasterDAL.SelectAllItemMasterPageWise(0, int.MaxValue, out TotalRecords);
            ...
            string file = System.Configuration.ConfigurationManager.AppSettings["FileSavePath"] + "Item.csv";

            var parameterNames = typeof(loanItemMasterDAL).GetProperties()
                .Where(p => !p.GetGetMethod().IsVirtual)
                .Select(p => p.Name)
                .ToArray();

            bool IsSuccess = loanAppGlobals.ExportCsv(lstItemMaster, parameterNames, parameterNames, file);
            if (!IsSuccess)
            {
