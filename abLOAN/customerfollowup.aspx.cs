    ...
    public partial class customerfollowup : BasePage
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                objCustomerFollowupDAL.linktoGuarantorMasterId = Convert.ToInt32(hdnGuarantorMasterId.Value);
        ...
        protected void lvCustomerFollowedup_ItemCommand(object sender, ListViewCommandEventArgs e)
        {
            try
            {
                ...
                if(sender == lvCustomerFollowedup) {
        ...
        protected void lvFollowupNote_ItemCommand(object sender, ListViewCommandEventArgs e)
        {
            try
            {
               
                
                    if (e.CommandName.Equals("DeleteNoteRecord", StringComparison.CurrentCultureIgnoreCase))
                    {
                        loanUser.CheckRoleRights(loanRoleRights.DeleteRecord);

                        loanFollowupNoteDAL objFollowupNoteDAL = new loanFollowupNoteDAL();
                        objFollowupNoteDAL.FollowupNoteId = Convert.ToInt32(((ListView)sender).DataKeys[e.Item.DataItemIndex].Value);
                        objFollowupNoteDAL.SessionId = ((loanUser)Session[loanSessionsDAL.UserSession]).SessionId;

                        loanRecordStatus rsStatus = objFollowupNoteDAL.DeleteCustomerfollowupNote();
                        if (rsStatus == loanRecordStatus.Success)
                        {
                            loanAppGlobals.ShowMessage(loanMessagesDAL.DeleteSuccess, loanMessageIcon.Success);
                        loanAppGlobals.ShowMessage(loanMessagesDAL.DeleteSuccess, loanMessageIcon.Success);
        ...
        protected void lvCustomerFollowedup_ItemDataBound(object sender, ListViewItemEventArgs e)
        {
            try
            {
                if (e.Item.ItemType == ListViewItemType.DataItem)
                {
                    ...
                    Label lblGuarantorName = (Label)e.Item.FindControl("lblGuarantorName");
                    lblGuarantorName.Text = objCustomerFollowupDAL.GuarantorName;
        ...
        private void GetFollowdupCustomer(int FollowupCustomerId)
        {
            ...
            hdnGuarantorMasterId.Value = objCustomerFollowupDAL.linktoGuarantorMasterId.ToString();
            txtSearchGuarantorIdNo.Text = objCustomerFollowupDAL.GuarantorIdNo;
            txtGuarantorName.Text = objCustomerFollowupDAL.GuarantorName;
            txtGuarantorIdNo.Text = objCustomerFollowupDAL.GuarantorIdNo;
