...
namespace abLOAN
{
    public partial class login : BasePage
    {
        ...
        protected   void btnLogin_Click(object sender, EventArgs e)
        {
            ...
            {
                await PerformLoginAsync(  sender,   e);
            }));
        }

        private async Task PerformLoginAsync(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else if (!string.IsNullOrEmpty(objUserMasterDAL.Email))
                {
                    ...
                    if (!await objUserMasterDAL.SendVerificationCode())
                    {
                        lblMessage.Text = Messages.ResourceManager.GetString("InvalidUsername");
                        pnlMessage.Visible = true;
                        txtUsername.Focus();
        ...
        protected void btnVerify_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(objUserMasterDAL.Email) && (!objUserMasterDAL.VerificationCode.Equals(txtVerificationCode.Text, StringComparison.InvariantCulture) || 
                    objUserMasterDAL.VerificationCodeDateTime < loanGlobalsDAL.GetCurrentDateTime().AddMinutes(-20)))
                {
