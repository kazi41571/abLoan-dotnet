...
namespace abLOAN
{
    public partial class bank : BasePage
    {
        ...
        protected void btnDownload_Click(object sender, EventArgs e)
        {
            ...
            objBankMasterDAL.IsEnabled = ddlFilterIsEnabled.SelectedValue == "Yes";
            objBankMasterDAL.linktoCompanyMasterId = ((loanUser)Session[loanSessionsDAL.UserSession]).CompanyMasterId;

            var verified = ddlFilterVerification.SelectedValue;
            if (verified == "Yes")
            {
            ...
            int TotalRecords;
            List<loanBankMasterDAL> lstBankMaster = objBankMasterDAL.SelectAllBankMasterPageWise(0, int.MaxValue, out TotalRecords);
            ...
            string file = System.Configuration.ConfigurationManager.AppSettings["FileSavePath"] + "Bank.csv";

            var parameterNames = typeof(loanBankMasterDAL).GetProperties()
                .Where(p => !p.GetGetMethod().IsVirtual)
                .Select(p => p.Name)
                .ToArray();

            bool IsSuccess = loanAppGlobals.ExportCsv(lstBankMaster, parameterNames, parameterNames, file);
            if (!IsSuccess)
            {
